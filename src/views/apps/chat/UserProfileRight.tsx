          ...
          <Box sx={{ position: 'relative' }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', padding: theme => theme.spacing(5) }}>
              <Box sx={{ mb: 5.5, display: 'flex', justifyContent: 'center' }}>
                ...
                >
                  {store.selectedChat.contact.imagenUsuario ? (
                    <MuiAvatar
                      sx={{ width: '5rem', height: '5rem' }}
                      src={store.selectedChat.contact.imagenUsuario}
                      alt={store.selectedChat.contact.nombre}
                    />
                  ) : (
                    <CustomAvatar
                      skin='light'
                    ...
                    >
                      {getInitials(store.selectedChat.contact.nombre)}
                    </CustomAvatar>
                  )}
                </Badge>
              </Box>
              <Typography sx={{ mb: 0.5, fontWeight: 500, textAlign: 'center' }}>
                {store.selectedChat.contact.nombre}
              </Typography>
            </Box>
          </Box>
                  ...
                  <List dense sx={{ p: 0 }}>
                    <ListItem sx={{ px: 2 }}>
                      ...
                      <ListItemText
                        ...
                        primary={`${store.selectedChat.contact.correo ? store.selectedChat.contact.correo : ''}`}
                      />
                    </ListItem>
                    <ListItem sx={{ px: 2 }}>
                      <ListItemIcon sx={{ mr: 2, color: 'text.primary' }}>
